...
export default function HomeScreen() {
  const { activeGoal, checklists, dateGoals, addDateGoal, addChecklist, renameChecklist, toggleTask, addTask } = useChecks();
  ...
  return <SafeAreaView style={styles.safe} edges={['top']}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    {[...checklists].sort((first, second) => Number(isChecklistComplete(first)) - Number(isChecklistComplete(second))).map((list) => <ChecklistProgressCard key={list.id} list={list} onToggleTask={(task) => updateTask(list.id, task.id, task.title, !task.done)} onAddTask={() => setTaskList(list.id)} onRename={() => { setEditingList(list.id); setEditedTitle(list.title); }} />)}
